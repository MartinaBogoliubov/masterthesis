...
\todo{figure to visualize to splitting of A(t)}
%
%
%
...
Inserting equation \eqref{eq: identity expectation value} and \eqref{eq: Kubo-identity} in the Kubo relaxation function yield the wanted form of the Kubo relaxation function, where on the right hand side of the following compuation is integrated by parts, dedicated with PI.
%
\begin{align}
	&\Phi_{\mt{AB}}(t) = \frac{i}{\hbar} \lim\limits_{s \to 0} \int\limits_{t}^{\infty} \dd{\tau} \expval{\comm{\mt{A}_{\mt{I}}(\tau)}{\mt{B}_{\mt{I}}(0)}}_{0} e^{-s\tau}
	\notag \\
...
The contribution of the semi-circle vanishs because it is assumed that $\chi_{\mt{AB}}(\omega)$ decrease very fast for large values of $\omega$.
...
The dissipative susceptibility of the form \todo{Why do we choose this typ of $\chi''(t-t')$}
%
\begin{align}
...
\todo{Motivation why we choose this kind of a scalar product}
...
\todo{figure of the projector's acting}
...
The defined correlation function is proportional to the Kubo relaxation function, which describes the system's reaction on a switched off pertubation, how we learned in section \ref{subsec: Kubo relaxation function}. \todo{write more to the goal of the memory matrix formalsim}
...
\todo{write that the founded expression is an exact one}
