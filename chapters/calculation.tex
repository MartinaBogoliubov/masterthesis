...
%
\begin{align}
	\mt{I}(z) :=  -\int\limits_{0}^{\infty} \dd{t} e^{iz t} \expval{\comm{\dot{\mt{P}}(t)}{\dot{\mt{P}}(0)}}_{\mt{H}_{1}} = \int\limits_{0}^{\beta} \dd{\tau} e^{z \tau} \expval{\mathcal{T}_{\tau} \dot{\mt{P}}(\tau) \dot{\mt{P}}(0)}_{\mt{H}_{1}}
\end{align}
%
The norm of the Jacobi determinate is $1$.
Further each derivative yields an $i$ which gives us in total one minus.
To symbolisied clearly that the frequency is an arbitrary complex number the variable $z$ is used instead of $\omega$ at this point.
...
%
\begin{align}
	\mt{I}(z) = \int\limits_{0}^{\beta} \dd{\tau} e^{z \tau} \expval{\mathcal{T}_{\tau} \dot{\mt{P}}(\tau) \dot{\mt{P}}(0)}_{\mt{H}_{0}}
\end{align}
%
...
%
\begin{align}
	\dot{\mt{P}}_{x}(\tau) = \frac{i}{\hbar} \sum\limits_{\vb{K}} \mt{J}_{\vb{K}} \int_{\vb{k}} K_{x} \Phi_{\mu}(\vb{k},\tau) \Phi_{\mu}(-\vb{k} - \vb{K},\tau)
\end{align}
%
where the direction of the momentum is chosen to the $x$-direction without loss of generality.
The sum over $\mu$ should be implied.
Inserting the time derivative of the momentum in $\mt{I}(z)$ yields
%
\begin{align}
	\mt{I}_{xx}(z) &= 
		-\frac{1}{\hbar^{2}} 
		...
		\notag \\
		&\times
		\expval{\mathcal{T}_{\tau} \Phi_{\mu}(\vb{k}_{1},\tau) \Phi_{\mu}(-\vb{k}_{1} - \vb{K}_{1},\tau) \Phi_{\mu}(\vb{k}_{2},0) \Phi_{\mu}(-\vb{k}_{2} - \vb{K}_{2},0)}_{\mt{H}_{0}}
\end{align}
%
...
Beside the integral, which is calculated in the section above, the static susceptibility is the second quantity.
...
Before the operators are inserted into the expectation value let us thinking about the possible combinations in the diagrammatic language.
...
Therefore the bosonic propagator yields always one disconnected diagram.
...
Thus many diagrams of the investigated are disconnected.
Only two ones aren't disconnected.
...
These two expectation value of four fermionic operators can't be solved directly.
...
Two contraction are possible for each expectation value in the case above, where one of them vanishes, if the time argument of the contracted operator is the same.
%
\begin{align}
	\chi_{\mt{PJ}}(\omega = 0) &= 
		\frac{1}{\hbar} 
...
The only important fact is that all singularities of the Green function has to be included by the contour.
...
Consider that the integrand has a singularity of second order at $z_{0} = \epsilon_{\alpha}(\vb{k})$.
%
\begin{align}
	\chi_{\mt{PJ}}(\omega = 0) &= 
		\frac{1}{\hbar} 
